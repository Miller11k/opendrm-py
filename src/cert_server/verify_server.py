...
def verify_root_cert(path: Path, exp: Expect) -> bool:
    ...
    # These fields MUST be False for a Root CA
    _assert(not ku.content_commitment, "content_commitment should be False")
    _assert(not ku.key_encipherment, "key_encipherment should be False")
    _assert(not ku.data_encipherment, "data_encipherment should be False")
    _assert(not ku.key_agreement, "key_agreement should be False")

    # Do NOT access encipher_only/decipher_only when key_agreement=False
...
def verify_intermediate_cert(path: Path, root_cert_path: Path, exp: Expect) -> bool:
    ...
    _assert(not ku.content_commitment, "Unexpected KU")
    _assert(not ku.key_encipherment, "Unexpected KU")
    _assert(not ku.data_encipherment, "Unexpected KU")
    _assert(not ku.key_agreement, "Unexpected KU")
    return True
