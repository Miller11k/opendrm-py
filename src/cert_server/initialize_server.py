...
def _self_signed_root(common_name: str, days: int = 3650):
    ...
    builder = (
        ...
        .add_extension(KeyUsage(    # Set certificate's key usage
            ...
            encipher_only=None,        # Only valid if key_agreement=True, not applicable       # type: ignore[arg-type]
            decipher_only=None         # Only valid if key_agreement=True, not applicable       # type: ignore[arg-type]
        ), critical=True)
    )
...
def _pem_privkey(key, passphrase: bytes | None = None) -> bytes:
    ...
    return key.private_bytes(
        ...
        format=serialization.PrivateFormat.PKCS8,  # Set format to PKCS8
...
def _sign_intermediate(root_key, root_cert, common_name: str, days: int = 1825):
    ...
    crl = (
        x509.CertificateRevocationListBuilder()
        .issuer_name(subject)                               # The Root CA is the issuer of this Intermediate CA certificate
...
def _sign_leaf(
    ...
    builder = (
        ...
        .add_extension(
            ...
            KeyUsage(
                ...
                encipher_only=None,      # Only valid if key_agreement=None     # type: ignore[arg-type]
                decipher_only=None       # Only valid if key_agreement=None     # type: ignore[arg-type]
            ),
            critical=True
        )
        .add_extension(
    ...
    if san_dns:
        builder = builder.add_extension(
            SubjectAlternativeName([DNSName(d) for d in san_dns]),
            critical=False
        )
